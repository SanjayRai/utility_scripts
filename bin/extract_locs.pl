...
$USAGE = "\n Usage :\n\t extract_locs file.ngd file.ncd [-RLOC] [-NO_FF] [-NO_BRAM] [-NO_DSP] [-Iinstance_name_to_extract] [-Einstance_path_to_exclude1#instance_path_to_exclude2]\n";
...
if ($#ARGV > 1) {
    for ($i = 2; $i < ($#ARGV+1); $i++) {
        ...
        if ($input_arg=~/^-RLOC/i) {
            $WRITE_RLOCS = 1;
            $out_file = $design_name."_extract_locs_RLOC.ucf";
        } elsif ($input_arg=~/^-NO_FF/i) {
            $WRITE_FFS = 0;
        } elsif ($input_arg=~/^-NO_BRAM/i) {
            $WRITE_BRAMS = 0;
        } elsif ($input_arg=~/^-NO_DSP/i) {
            $WRITE_DSPS = 0;
        } elsif ($input_arg=~/^-I/) {
...
print "Instance being Extracted to Constraint File : $include_instances\n";
...
if ($file_ext_ngd ne "ngd") { 
		...
		s/\s_.*?:.*:.*?\s/ /; # TurnOff greedy
			...
			$CFG_string = $CFG_string.$1." = ".$2.":";
                    ...
                    if ($token=~/([ABCD]\dLUT):.+:/) {
                        ...
                        $BEL = $1;
                    } elsif ($token=~/FIFO\d\d.*?:.+:/) {
                        $BEL = "BRAM";
                    } elsif ($token=~/RAMB.*?:.+:/) {
                        $BEL = "BRAM";
                    } elsif ($token=~/DSP48.*?:.+:/) {
                        $BEL = "DSP48";
                    } elsif ($token=~/MMCM_ADV:.+:/) {
                        $BEL = "PLL";
                    } elsif ($token=~/PLL_ADV:.+:/) {
                        $BEL = "PLL";
                    }
        ...
        foreach $hdl_instance_name ( sort (keys %hdl_instance_names)) {
            if( exists $primitives_from_xdl_file_loc{$hdl_instance_name} ) {
                ...
                if ($write_constraints == 1) {
                    ...
                    $ucf_instance_name = $hdl_instance_name;
                    if($write_bel_constraints == 0) {
                        if ($WRITE_RLOCS == 1) {
                            $ucf_instance_name =~ s/$include_instances?(.*)/$1/; #Turn off greedy matching
                            $xdl_primitive_rloc = $xdl_primitive_loc;
                            $xdl_primitive_rloc =~ s/.*_(X\d+Y\d+)/$1/;
                            print FP_out "INST \"$ucf_instance_name\" RLOC = \"$xdl_primitive_rloc\";$primitives_from_xdl_file_comment{$xdl_primitive_loc}\n";
                        } else {
                            print FP_out "INST \"$ucf_instance_name\" LOC = \"$xdl_primitive_loc\";$primitives_from_xdl_file_comment{$xdl_primitive_loc}\n";
                        }
                    } else {
                        ...
                        if ($WRITE_RLOCS == 1) {
                            ...
                            $xdl_primitive_rloc = $xdl_primitive_loc;
                            $xdl_primitive_rloc =~ s/.*_(X\d+Y\d+)/$1/;
                            print FP_out "INST \"$ucf_instance_name\" RLOC = \"$xdl_primitive_rloc\" | BEL = \"$xdl_primitive_bel\";$primitives_from_xdl_file_comment{$xdl_primitive_loc}\n";
                        } else {
                            print FP_out "INST \"$ucf_instance_name\" LOC = \"$xdl_primitive_loc\" | BEL = \"$xdl_primitive_bel\";$primitives_from_xdl_file_comment{$xdl_primitive_loc}\n";
                        }
                    }
                } else {
    ...
    if ($INCLUDE_LUTS_IN_UCF == 1 ) {
        print FP_out "#Constrained LUT Count = $Constrained_LUT_count (Total LUTs = $Total_LUT_count)\n";
    }
    ...
    if ($INCLUDE_LUTS_IN_UCF == 1 ) {
        print "#Constrained LUT Count = $Constrained_LUT_count (Total LUTs = $Total_LUT_count)\n";
    }
