...
$Error_msg = "Wrong Arguments !\n\tusage:\n\t\txil_ip_gen_synp -lib Library_name synplify_prj_file.prj module_name io_ports.txt [[-sd core_search path1] [-sd core_seach_path2] ...]\n";
$Error_msg1 = "synplify_prj_file.prj : is the original synplify project file\n"; 
$Error_msg2 = "module_name : is any module in the design that needs to be packaged as an IP\n";
$Error_msg3 = "io_ports.txt : is a regular test file with one port per line that gets portedout to IO pins (clocks, HSIO, Memory IO etc)\n"; 
...
$module_synp_command_file = "$module_name"."_ip_gen_synp.prj";
$EDIF_file = "$library_name\/coregen\/$module_name".".edf";
...
open FP_SYNP_PRJ, "<$ARGV[2]" or die "Cannot Open $ARGV[2] file for reading : $!";


while (<FP_SYNP_PRJ>) {
    if (/^set_option\s*-part\s.*/i) {
	chomp;
        $DEVICE_TYPE_1 = $_;
        $DEVICE_TYPE_1 =~ s/^set_option\s*-part\s*(\w.*)/$1/;
    }
    if (/^set_option\s*-speed_grade\s.*/i) {
	chomp;
        $DEVICE_TYPE_2 = $_;
        $DEVICE_TYPE_2 =~ s/^set_option\s*-speed_grade\s*(-\d.*)/$1/;
    }
    if (/^set_option\s*-package\s.*/i) {
	chomp;
        $DEVICE_TYPE_3 = $_;
        $DEVICE_TYPE_3 =~ s/^set_option\s*-package\s*(\w.*)/$1/;
    }
}
close FP_SYNP_PRJ;

$DEVICE_TYPE = $DEVICE_TYPE_1.$DEVICE_TYPE_2."-".$DEVICE_TYPE_3;
...
open FP_SYNP_PRJ, "<$ARGV[2]" or die "Cannot Open $ARGV[2] file for reading : $!";
open FP_IP_SYNP, ">$module_synp_command_file" or die "Cannot Create intermedeate Synplify Project  file $module_synp_command_file: $!";

while (<FP_SYNP_PRJ>) {
    if (/^project\s*-result_file\s.*/i) {
        print FP_IP_SYNP "project -result_file $library_name\/coregen\/$module_name\n";
    } elsif (/^add_file\s*-constraint/i) {
        print FP_IP_SYNP "add_file -constraint \"\"\n";
    } elsif (/^set_option\s*-top_module\s.*/i) {
        print FP_IP_SYNP "set_option -top_module \"$module_name\"\n";
    } elsif (/^set_option\s*-write_apr_constraint\s.*/i) {
        print FP_IP_SYNP "set_option -write_apr_constraint 0\n"; # DO NOT write TIMESPEC into the ngc as these are in the global Namespace and might conflict
    } elsif (/^set_option\s*-disable_io_insertion\s.*/i) {
        print FP_IP_SYNP "set_option -disable_io_insertion 1\n";
    } elsif (/^set_option\s*-library_path\s/i) {
        print FP_IP_SYNP;
    } else {
        print FP_IP_SYNP;
    }
...
print "\tsynplify_pro $module_synp_command_file\n";
system("synplify_pro $module_synp_command_file") == 0 or die "\tSynplify Synthesis Failed! : $?\n";
wait;
print "\t******* running NGDBUILD on $module_name ********\n";

system("rm -rf $library_name\/xst_tmp\/*");
print "\tngdbuild -verbose -p $DEVICE_TYPE $EDIF_file $NGD_file $core_search_path\n";
system("ngdbuild -verbose -p $DEVICE_TYPE $EDIF_file $NGD_file $core_search_path") == 0 or die "\tNGDBUILD Failed : $?\n"; 
...
push (@ip_gen_top_rtl,"    i_reset : in std_logic;\n");
push (@ip_gen_top_rtl,"    iTCK    : in std_logic;\n");
push (@ip_gen_top_rtl,"    iTDI    : in std_logic;\n");
push (@ip_gen_top_rtl,"    i_shift : in std_logic;\n");
...
push (@ip_gen_top_rtl,"    generic (WIDTH : integer := 100);\n");
push (@ip_gen_top_rtl,"    port (\n");
push (@ip_gen_top_rtl,"    o_reset : in std_logic;\n");
push (@ip_gen_top_rtl,"    oTCK    : in std_logic;\n");
push (@ip_gen_top_rtl,"    oTDO    : out std_logic;\n");
push (@ip_gen_top_rtl,"    o_shift : in std_logic;\n");
...
push (@ip_gen_top_rtl,"signal iTCK, iTDI, i_shift : std_logic := '0';\n");
push (@ip_gen_top_rtl,"signal oTCK, oTDO, O_shift : std_logic := '0';\n");
push (@ip_gen_top_rtl,"\n");
...
push (@ip_gen_top_rtl,"U_BSCAN_i: BSCAN_VIRTEX5\n");
push (@ip_gen_top_rtl,"\tgeneric map\n");
push (@ip_gen_top_rtl,"\t(\n");
push (@ip_gen_top_rtl,"\t\tJTAG_CHAIN => 1\n");
push (@ip_gen_top_rtl,"\t)\n");
push (@ip_gen_top_rtl,"\tport map\n");
push (@ip_gen_top_rtl,"\t(\n");
push (@ip_gen_top_rtl,"\t\tCAPTURE => open,\n");
push (@ip_gen_top_rtl,"\t\tDRCK => iTCK,\n");
push (@ip_gen_top_rtl,"\t\tRESET => OPEN,\n");
push (@ip_gen_top_rtl,"\t\tSEL => OPEN,\n");
push (@ip_gen_top_rtl,"\t\tSHIFT => i_shift,\n");
push (@ip_gen_top_rtl,"\t\tTDI => iTDI,\n");
push (@ip_gen_top_rtl,"\t\tUPDATE => OPEN,\n");
push (@ip_gen_top_rtl,"\t\tTDO => '0'\n");
push (@ip_gen_top_rtl,"\t);\n\n");
push (@ip_gen_top_rtl,"U_JTAG_USER_REG_i: jtag_user_reg_i\n");
push (@ip_gen_top_rtl,"\tgeneric map (WIDTH => $number_of_inputs)\n");
push (@ip_gen_top_rtl,"\tport  map (\n");
push (@ip_gen_top_rtl,"\t\ti_reset => xil_ip_top_reset,\n");
push (@ip_gen_top_rtl,"\t\tiTCK    => iTCK,\n");
push (@ip_gen_top_rtl,"\t\tiTDI    => iTDI,\n");
push (@ip_gen_top_rtl,"\t\ti_shift => i_shift,\n");
push (@ip_gen_top_rtl,"\t\tjtag_in_reg => jtag_in_reg);\n\n");

push (@ip_gen_top_rtl,"-- Output Boundary SCAN for OUTPUT Ports\n\n");
push (@ip_gen_top_rtl,"U_BSCAN_o: BSCAN_VIRTEX5\n");
push (@ip_gen_top_rtl,"\tgeneric map\n");
push (@ip_gen_top_rtl,"\t(\n");
push (@ip_gen_top_rtl,"\t\tJTAG_CHAIN => 2\n");
push (@ip_gen_top_rtl,"\t)\n");
push (@ip_gen_top_rtl,"\tport map\n");
push (@ip_gen_top_rtl,"\t(\n");
push (@ip_gen_top_rtl,"\t\tCAPTURE => open,\n");
push (@ip_gen_top_rtl,"\t\tDRCK => oTCK,\n");
push (@ip_gen_top_rtl,"\t\tRESET => OPEN,\n");
push (@ip_gen_top_rtl,"\t\tSEL => OPEN,\n");
push (@ip_gen_top_rtl,"\t\tSHIFT => o_shift,\n");
push (@ip_gen_top_rtl,"\t\tTDI => OPEN,\n");
push (@ip_gen_top_rtl,"\t\tUPDATE => OPEN,\n");
push (@ip_gen_top_rtl,"\t\tTDO => oTDO\n");
push (@ip_gen_top_rtl,"\t);\n\n");
push (@ip_gen_top_rtl,"U_JTAG_USER_REG_o: jtag_user_reg_o\n");
push (@ip_gen_top_rtl,"\tgeneric map (WIDTH => $number_of_outputs)\n");
push (@ip_gen_top_rtl,"\t port map (\n");
push (@ip_gen_top_rtl,"\t\to_reset => xil_ip_top_reset ,\n");
push (@ip_gen_top_rtl,"\t\toTCK    => oTCK,\n");
push (@ip_gen_top_rtl,"\t\toTDO    => oTDO,\n");
push (@ip_gen_top_rtl,"\t\to_shift => o_shift,\n");
...
print FP_IP_GEN_TOP_UCF "NET \"iTCK\" TNM_NET = \"iTCK\";\n";
print FP_IP_GEN_TOP_UCF "NET \"oTCK\" TNM_NET = \"oTCK\";\n\n";
...
print FP_IP_GEN_TOP_UCF "TIMEGRP \"xil_ip_top_reset_PAD\" = PADS (\"xil_ip_top_reset\");\n";
print FP_IP_GEN_TOP_UCF "TIMESPEC \"TS_tig_xil_ip_top_reset\" = FROM \"xil_ip_top_reset_PAD\" 1000 ns DATAPATHONLY;\n\n";
